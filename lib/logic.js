...
/**
 * Sample transaction processor function.
 * @param {org.example.basic.producerToLogistics} prodToLog The sample transaction instance.
 * @transaction
 */
        ...
        return getAssetRegistry('org.example.basic.Producer')
            .then(function(playerRegistryHarvest) {
                ...
                if (!exists && existsHarvest) {
                    ...
                    throw Error('Companhia de Logistica não registada !')
                    ...
                    if (exists && existsHarvest) {
                        getAssetRegistry('org.example.basic.Batch')
                            .then(function(assetRegistry) {
                                ...
                                if (prodToLog.batch.previousOwners.length <= 0) {

                                    prodToLog.batch.previousOwners.push(prodToLog.batch.owner)

                                } else {
                                    prodToLog.batch.previousOwners.push(prodToLog.batch.issuer)
