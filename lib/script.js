...
function quality(quality) {
    ...
    var expirationDate = new Date(quality.quality.newBatch.expirationDate);
     ...
     return getAssetRegistry('org.doptrace.Industry_Retailer').then(function(playerRegistry) {
       ...
       }).then(async function(existsBatch) {
             
             let updateRegistry = await getAssetRegistry('org.doptrace.Batch');
    ...
    throw Error('Erro na transação');
        ...
        quality.quality.assessedBatch.state = 'DISABLED'
    
    	updateRegistry.update(quality.quality.assessedBatch);   
                        ...
                        for( i=0 ; i < batchLength; i++){
                            ...
                            if(transformation.transformation.usedBatches[i].amount > transformation.transformation.amountPerBatch[i]){
                              ...
                              newBatch.previousEvents = transformation.transformation.usedBatches[i].previousEvents.concat(array)
              					...
              					transformation.transformation.usedBatches[i].state = 'DISABLED'
    
                                await registryBatch.update(transformation.transformation.usedBatches[i]);  
...
async function sale(sale){
  ...
  var now = new Date();
  var expirationDate = new Date(sale.sale.newBatch.expirationDate);
                   ...
                   await saleRegistry.add(newAsset);
                      ...
                      for( i=0 ; i < batchLength; i++){
                          ...
                          if(sale.sale.soldBatches[i].amount > sale.sale.amountPerBatch[i]){
                            ...
                            if(sale.sale.buyer == null){
                              
                              newBatch.currentOperator = sale.sale.seller
                            ...
                            //Criação do batch de sobra
                              ...
                              var newBatch = factory.newResource(
                                  'org.doptrace', 
                                  'Batch',
                                  sale.sale.soldBatches[i].id + 'Sobra'); 
                        	...
                        	sale.sale.soldBatches[i].state = 'DISABLED'
    
                            await registryBatch.update(sale.sale.soldBatches[i]);  
